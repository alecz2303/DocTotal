...
new
    ...
    class extends Component
    {
        ...
        {
            $source = app(RepeatPrescription::class)->source($uuid);
            $this->sourceUuid = $source->uuid;
            $this->patientId = $source->patient_id;
            $this->prescribed_at = now()->format('Y-m-d\\TH:i');
            $this->general_instructions = $source->general_instructions ?? '';
            $this->items = $source->items->map(fn ($item) => [
                'search' => '',
        ...
        #[Computed]
        public function source(): Prescription
        {
            return app(RepeatPrescription::class)->source($this->sourceUuid, $this->patientId);
        }
        ...
        {
            app(RepeatPrescription::class)->authorize();
            if ($this->createdUuid !== null) {
                $this->redirectRoute('prescriptions.show', ['uuid' => $this->createdUuid]);
                return;
            }

            $new = app(RepeatPrescription::class)->execute(
                $this->sourceUuid,
                $this->patientId,
                [
            ...
            $this->createdUuid = $new->uuid;
            session()->flash('success', 'Nueva receta registrada. La receta origen permanece intacta.');
            $this->redirectRoute('prescriptions.show', ['uuid' => $new->uuid]);
        }
...
<div class="mx-auto max-w-6xl">
    ...
    {{-- HEADER --}}
    <div class="mb-6">
        <a href="{{ route('prescriptions.show', ['uuid' => $sourceUuid]) }}"
                ...
                <div class="flex min-w-0 items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <h1 class="text-2xl font-bold tracking-tight text-slate-950">Repetir receta</h1>
                        ...
                        <p class="mt-1 truncate font-semibold text-slate-700">
                            {{ $this->source->patient->first_name }}
                            {{ $this->source->patient->last_name }}
                        </p>

                        <p class="mt-1 text-sm text-slate-500">
                            Basada en la receta del
                            {{ $this->source->prescribed_at->locale('es')->translatedFormat('d \d\e F \d\e Y') }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="mb-6 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
        Revisa y ajusta el tratamiento antes de guardar. Se creará una receta nueva para el mismo paciente,
        con fecha actual precargada y sin asociación a la consulta anterior. La receta origen no se modificará.
    </div>
